/* ============================================================
   SIMULADOR CONSÓRCIO NACIONAL BAJAJ — script.js (v2 final)
   Imagens: assets/{modelo}/{cor}.{extensao}
   webp: pulsar-n150, dominar-ns400z | png: demais
   ============================================================ */

/* ---------- CONFIGURAÇÃO DA PLANILHA (links publicados) ---------- */
const CONFIG = {
...
/* ---------- MODELOS (cores, extensão e ficha técnica oficial Bajaj) ---------- */
const MODELOS = {
  "pulsar-n150": {
    nome: "Pulsar N150",
    cores: ["branca", "preta", "vermelha"],
    ext: "webp",
    ficha: {
...
/* ---------- TABELAS (38 linhas, idênticas à planilha) ----------
   [modelo, grupo, plano, variacao, codigo, credito, parcela] */
const TABELAS_RAW = [
...
/* Carrega a imagem correta com fallback de extensão */
function carregarImagem(el, pasta, cor) {
  const info = MODELOS[pasta];
  const exts = info ? [info.ext, info.ext === "webp" ? "png" : "webp"] : ["png", "webp"];
  let i = 0;
  const tentar = function () {
    if (i >= exts.length) { el.src = "assets/logos/logo-bajaj-horizontal.png"; return; }
...
function renderModelos() {
  ...
  if (!container) return;
  container.innerHTML = Object.entries(MODELOS).map(function (par) {
    const id = par[0], m = par[1];
    return '<div class="modelo-card" data-modelo="' + id + '">' +
      '<p class="modelo-card__nome">' + m.nome + '</p>' +
      '<p class="modelo-card__cores">' + m.cores.join(" · ") + '</p>' +
      '</div>';
  }).join("");

  container.querySelectorAll(".modelo-card").forEach(function (card) {
    card.addEventListener("click", function () {
      container.querySelectorAll(".modelo-card").forEach(function (c) { c.classList.remove("selecionado"); });
...
function renderPlanos() {
  ...
  container.innerHTML = combos.map(function (c) {
    ...
    return '<div class="plano-card" data-grupo="' + c.grupo + '" data-plano="' + c.plano + '">' +
  ...
  container.querySelectorAll(".plano-card").forEach(function (card) {
    card.addEventListener("click", function () {
      container.querySelectorAll(".plano-card").forEach(function (c) { c.classList.remove("selecionado"); });
      card.classList.add("selecionado");
...
/* ---------- ETAPA 5: MÉDIA DO MENOR LANCE (único indicador) ---------- */
...
function renderResumo() {
  ...
  $("cardResumo").innerHTML = linhas.map(function (l) {
    return '<div class="resumo__linha"><span class="resumo__label">' + l[0] + '</span><span class="resumo__valor">' + l[1] + '</span></div>';
  }).join("");
}
...
function bindNavegacao() {
  $("btnAvancarModelo").addEventListener("click", function () { renderCores(); irParaEtapa(2); });
  $("btnAvancarCor").addEventListener("click", function () { renderVisualizacao(); irParaEtapa(3); });
  $("btnAvancarVisualizacao").addEventListener("click", function () { renderPlanos(); irParaEtapa(4); });
  $("btnAvancarGrupo").addEventListener("click", function () { renderMedia(); irParaEtapa(5); });
  $("btnAvancarMedia").addEventListener("click", function () { irParaEtapa(6); });
  ...
  document.querySelectorAll("[data-voltar]").forEach(function (btn) {
    btn.addEventListener("click", function () { irParaEtapa(parseInt(btn.dataset.voltar, 10)); });
  });
}
...
/* ---------- CARREGAR DADOS DA PLANILHA (fallback local) ---------- */
async function carregarDadosPlanilha() {
  try {
    const resp = await fetch(CONFIG.urlAtual);
    if (!resp.ok) throw new Error("falha");
    const dados = {};
    parseCSV(await resp.text()).forEach(function (l) {
      const k = l[0], v = l[1];
  ...
  try {
    ...
    for (let i = 1; i < linhas.length; i++) {
      const nome = linhas[i][0], codigo = linhas[i][1], grupo = linhas[i][2],
            plano = linhas[i][3], credito = linhas[i][4], parcela = linhas[i][5];
      if (!nome || !codigo || !grupo) continue;
      const chave = String(nome).toLowerCase().replace(/[^a-z0-9]/g, "").replace(/\+?le/gi, "").replace(/120%/g, "");
      const modelo = mapaModelo[chave];
      if (!modelo) continue;
      const variacao = /\+ ?le/i.test(nome) ? "+LE" : /120%/.test(nome) ? "120%" : "";
